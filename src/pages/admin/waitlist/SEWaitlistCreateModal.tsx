...
import type { CreateSEWaitlistEntryData, RegisteredUser } from '@/hooks/data/useSpecialEventWaitlist';
import { useSpecialEventWaitlist } from '@/hooks/data/useSpecialEventWaitlist';

interface SEWaitlistCreateModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  specialEvents: SpecialEvent[];
  onSave: (data: CreateSEWaitlistEntryData) => Promise<{ id: string; position: number }>;
  preselectedSpecialEventId?: string;
}

export const SEWaitlistCreateModal: FC<SEWaitlistCreateModalProps> = ({
  open,
  onOpenChange,
  specialEvents,
  onSave,
  ...
  const { getRegisteredUsers } = useSpecialEventWaitlist();

  // Fetch users when modal opens
  useEffect(() => {
    if (open) {
      setSpecialEventId(preselectedSpecialEventId || '');
  ...
  }, [open, preselectedSpecialEventId, getRegisteredUsers]);
  ...
  const handleSave = async () => {
    // Validation
    if (!specialEventId) {
      setError('Please select a special event');
      return;
    }
    if (!selectedUser) {
    ...
    try {
      ...
      const event = specialEvents.find((e) => e.id === specialEventId);
      setSuccessMessage(
        `${selectedUser.fullName} added to ${event?.title || 'special event'} waitlist at position #${result.position}`
      );
      ...
      console.error('Error creating special event waitlist entry:', err);
      const errorMessage = err instanceof Error ? err.message : 'Failed to add to waitlist';
      // Check for duplicate entry error
      if (errorMessage.includes('duplicate') || errorMessage.includes('unique')) {
        setError('This user is already on the waitlist for this special event');
      } else {
      ...
      <DialogContent className="max-w-md bg-slate-900 border-slate-700">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-400">
            Select a registered user to add to a special event waitlist.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="special-event" className="text-white">
              Special Event <span className="text-red-400">*</span>
            </Label>
            <select
              id="special-event"
            ...
            >
              <option value="">Select a special event...</option>
